...
function parseSelectedValue(text) {
  const match = text.trim().match(/^(-?\d*\.?\d+)\s*(px|rem|vw|vh|dvw|dvh|lvw|lvh|svw|svh|vmin|vmax)$/i)
  if (!match) return null
  return { value: parseFloat(match[1]), unit: match[2].toLowerCase() }
}

function parseViewportValue(text) {
  const match = text.trim().match(/^(-?\d*\.?\d+)\s*(vw|vh|vmin|vmax|dvw|dvh|lvw|lvh|svw|svh)$/i)
  if (!match) return null
  return { value: parseFloat(match[1]), unit: match[2].toLowerCase() }
}
...
function parsePxRemOrTw(text) {
  ...
  if (!match) return null
  return { value: parseFloat(match[1]), unit: match[2] ? match[2].toLowerCase() : 'tw' }
}
...
function registerConverter(context) {
  const convertToViewportCmd = vscode.commands.registerCommand('viewportConverter.convertToViewportUnit', async () => {
    ...
    const picked = await vscode.window.showQuickPick(sortedScreens.map((s) => s.toString()), {
    ...
    await editor.edit((editBuilder) => {
      for (const sel of selections) {
        ...
        if (!text) {
          ...
          if (!parsedValue || !['px', 'rem'].includes(parsedValue.unit)) continue
          const range = new vscode.Range(sel.start.line, hit.start, sel.start.line, hit.end)
          const px = parsedValue.unit === 'rem' ? parsedValue.value * baseFontSize : parsedValue.value
    ...
    await editor.edit((editBuilder) => {
      for (const sel of selections) {
        ...
        const convertMatch = (value, unit) => {
          ...
          else if (unit === 'vmin') denom = Math.min(parsed.width, parsed.height)
          else if (unit === 'vmax') denom = Math.max(parsed.width, parsed.height)
        ...
        if (!text) {
          ...
          const range = new vscode.Range(sel.start.line, hit.start, sel.start.line, hit.end)
          editBuilder.replace(range, convertMatch(parsedValue.value, parsedValue.unit))
        } else {
          const replacements = []
          let unitMatch
          while ((unitMatch = unitRegex.exec(text)) !== null) {
            ...
            const startAbsolute = editor.document.offsetAt(sel.start) + startOffset
    ...
    await editor.edit((editBuilder) => {
      for (const sel of selections) {
        ...
        if (!text) {
          const hit = detectValueAtPosition(editor.document, sel.start)
          if (!hit) continue
          const match = hit.text.trim().match(/^(-?\d*\.?\d+)\s*(px|rem|vw|vh)$/i)
          if (!match) continue
  ...
  const codeActionProvider = {
    provideCodeActions(document, range) {
      const cursor = range?.start || vscode.window.activeTextEditor?.selection.start
      if (!cursor) return
  ...
  const quickConvertItem = vscode.window.createStatusBarItem(vscode.StatusBarAlignment.Left, 94)

  const updateStatus = () => {
    const screenValue = getConfig().get('lastUsedScreen', '1440x900')
    const base = getConfig().get('baseFontSize', 16)
    const precision = getConfig().get('viewportPrecision', 4)
    const screenText = (typeof screenValue === 'string' ? screenValue : String(screenValue)).replace('x', '×')
  ...
  const settingsCmd = vscode.commands.registerCommand('viewportConverter.viewportSettingsQuick', async () => {
    ...
    if (pick.action === 'screen') {
      const screens = cfg.get('viewportScreens', [])
      const chosen = await vscode.window.showQuickPick((screens || []).map(String), {
      ...
      const input = await vscode.window.showInputBox({
        ...
        validateInput: (value) => (/^\d+(\.\d+)?$/.test(value) && parseFloat(value) > 0) ? null : 'Enter a positive number',
      })
